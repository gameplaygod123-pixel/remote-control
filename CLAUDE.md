...
Latest release: **v1.18.3** (Parsec-style floating session controls — collapsible pill over full-window video; only the ⠿ grip is a drag region). v1.18.2 = single-instance lock (relaunch surfaces the running window). v1.18.1 = UI pass 2: agent preview box removed, agent window fixed 680×700; controller stays resizable). v1.18.0 = titlebar (OS bar hidden, single app-drawn bar
