...
// node-datachannel, depacketizes to Annex-B, and drives the Swift render binary
// (VideoToolbox decode -> AVSampleBufferVideoRenderer). A deliberate mirror of
...
// process so a decode/render crash respawns instead of taking down the Electron
// UI, and the caller can fall back to the WebRTC <video> path via isReady().
//
// Receiver-specific vs the sender: it consumes remote-OFFER and emits an ANSWER
// (the agent is still the offerer, as today); it takes set-render-rect so main
// can keep the native video window tracking the Electron session view; and it
// surfaces first-frame so the renderer can drop its "connecting..." overlay.
...
export function startVideoReceiverHost(callbacks: VideoReceiverCallbacks): VideoReceiverHost {
  ...
  // but we remember that a session is active + the last render rect so a
  // mid-session respawn re-arms the helper and repositions its window without
  // waiting on the caller. The fresh OFFER needed to actually re-negotiate comes
  // from the agent over signaling (the caller drives that via onDown -> re-pair).
  let sessionActive = false
  let lastRenderRect: RenderRect | null = null
    ...
    proc.on('message', (msg: VideoReceiverToMain) => {
      switch (msg.evt) {
        case 'ready':
          ...
          // answer the agent's next offer) and re-applies the window placement.
          if (sessionActive) {
            log('respawn: re-arming start-session')
            sendToChild({ cmd: 'start-session' })
            if (lastRenderRect) sendToChild({ cmd: 'set-render-rect', ...lastRenderRect })
          }
          break
        case 'pong':
          if (pongTimeout) clearTimeout(pongTimeout)
  ...
  return {
    ...
    setRenderRect: (rect: RenderRect) => {
      lastRenderRect = rect
      sendToChild({ cmd: 'set-render-rect', ...rect })
    },
    stopSession: () => {
      log('stopSession()')
      sessionActive = false
